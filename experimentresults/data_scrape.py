...
print("Timed out graphs written to %s" % (timeout_file))
...
for i in range(len(k_opt)):
	if k_opt[i] != k_gtrue[i] and time_out[i] == 0:
		print(name[i], k_opt[i], k_gtrue[i])
...
print("(3) The average runtime of non-trivial, non-timeouts is: %f" % (nontt_times/div))
